...
function insertNewGigInterview($pdo, $gig_id, $freelancer_id, $time_start, $time_end) {
    // Check if user is already scheduled
    ...
    if (hasInterviewTimeConflict($pdo, $gig_id, $time_start, $time_end)) {
        return false;
    }
...
// Function to check if there's a time conflict with existing interviews
function hasInterviewTimeConflict($pdo, $gig_id, $new_start, $new_end) {
    $sql = "SELECT * FROM gig_interviews 
            ...
            AND ((time_start BETWEEN ? AND ?) 
                ...
                OR (? BETWEEN time_start AND time_end))";
    $stmt = $pdo->prepare($sql);
    $stmt->execute([$gig_id, $new_start, $new_end, $new_start, $new_end, $new_start, $new_end]);
